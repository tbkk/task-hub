...
const orderId=ref(''),number=ref(''),receiver=ref(''),version=ref(0),requestStatus=ref(''),busy=ref(false),error=ref(''),done=ref(false),confirmKey=ref('')
function back(){uni.navigateBack()}
onLoad(q=>{orderId.value=String(q?.orderId||'');number.value=String(q?.number||'');receiver.value=String(q?.receiver||'');version.value=Number(q?.version||0);requestStatus.value=String(q?.request||'');confirmKey.value=newIdempotencyKey()})
async function confirm(){if(busy.value||done.value)return;busy.value=true;error.value='';try{await confirmPickup(orderId.value,version.value,confirmKey.value);done.value=true;uni.showToast({title:'取货已确认',icon:'success'})}catch(e){error.value=e instanceof Error?e.message:'确认失败，请刷新后重试'}finally{busy.value=false}}
</script>
