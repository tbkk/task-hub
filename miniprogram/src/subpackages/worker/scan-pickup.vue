...
import { scanVehicle, openPickup, confirmPickup } from '@/features/pickup/api'
...
async function open(order:PickupScan['orders'][number]){ busy.value=true; try { await openPickup(order.id,order.version,`pickup-open-${order.id}-${Date.now()}`); await confirmPickup(order.id,order.version,`pickup-confirm-${order.id}-${Date.now()}`); uni.showToast({title:'取货已确认',icon:'success'}); } catch(e){ error.value=e instanceof Error?e.message:'操作失败' } finally{busy.value=false} }
</script>
