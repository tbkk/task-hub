...
import { cancelTask, getDispatchBatch, getTask, goTask } from '@/features/control/api'
...
const id=ref(''),task=ref<VehicleTask|null>(null),batchVersion=ref<number|null>(null),loading=ref(true),busy=ref(false),error=ref(''),result=ref('')
async function load(){loading.value=true;error.value='';try{task.value=await getTask(id.value);batchVersion.value=(await getDispatchBatch(task.value.batchId)).version}catch(e){error.value=errorText(e)}finally{loading.value=false}}
async function control(action:'go'|'cancel'){if(!task.value||batchVersion.value===null||busy.value)return;busy.value=true;error.value='';try{const response=action==='go'?await goTask(task.value.id,batchVersion.value,newIdempotencyKey()):await cancelTask(task.value.id,batchVersion.value,newIdempotencyKey());result.value=controlStatusText(response.status);await load()}catch(e){error.value=errorText(e)}finally{busy.value=false}}
