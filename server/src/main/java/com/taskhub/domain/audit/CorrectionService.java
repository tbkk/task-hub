...
public class CorrectionService {
  ...
  public ReportModels.Correction append(Actor actor,String objectType,String objectId,ReportModels.CorrectionInput input) {
    ...
    var versions=db.query("SELECT correction_version FROM correction_record WHERE object_type=? AND object_id=? FOR UPDATE",(rs,n)->rs.getInt(1));
    int version=versions.stream().max(Integer::compareTo).orElse(0); int next=version+1;
