...
已实现身份权限、主数据、订单、仓库审批/组批及派发/工单/消息的部分后端；取货控制、剩余页面和真实接入尚未完成。准确进度见 [总体路线图](docs/superpowers/plans/2026-09-20-development-roadmap.md)。

本机实验可运行 `./deploy.sh deploy`，部署管理平台到 `http://127.0.0.1:18090/`、小程序 H5 到 `http://127.0.0.1:18091/`，支持 restart/stop/status/logs。配置和首次准备见[本地部署说明](deploy/README.md)；无需 Docker。
